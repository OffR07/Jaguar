    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-emerald-950">
      <Card className="w-full max-w-md space-y-6">
        ...
        <div className="space-y-4">
          ...
          {view !== 'recovery' && (
            <>
              <Input label="Senha" type="password" placeholder="••••••••" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
              {view === 'register' && <Input label="Confirmar Senha" type="password" placeholder="••••••••" value={formData.confirm} onChange={e => setFormData({...formData, confirm: e.target.value})} />}
            </>
          )}

          {view === 'recovery' && <Input label="Código enviado por e-mail" placeholder="123456" value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} />}
        </div>

        <Button onClick={handleSubmit} loading={loading} className="w-full py-5 text-lg">
          {view === 'login' ? 'Entrar na Arena' : view === 'register' ? 'Finalizar Cadastro' : 'Validar Código'}
        </Button>

        <div className="flex justify-between px-2">
          {view === 'login' && <button onClick={() => setView('recovery')} className="text-[10px] font-black text-emerald-400 uppercase">Esqueci a senha</button>}
